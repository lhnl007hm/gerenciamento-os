...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.execute("INSERT INTO usuarios (username, senha, nome, email, contrato_id, role) VALUES ('admin', 'Admin@2024#Seguro', 'Administrador', 'admin@compass.com.br', 1, 'admin')")
        cursor.execute("INSERT INTO usuarios (username, senha, nome, email, contrato_id, role) VALUES ('gerente', 'Gerente@2024#Seguro', 'Gerente', 'gerente@compass.com.br', 1, 'gerente')")
        cursor.execute("INSERT INTO usuarios (username, senha, nome, email, contrato_id, role) VALUES ('tecnico', 'Tecnico@2024#Seguro', 'Técnico', 'tecnico@compass.com.br', 1, 'tecnico')")
...
def admin_novo_contrato():
    if request.method == 'POST':
        ...
        try:
            ...
            if 'ativo' in colunas:
                conn.execute("INSERT INTO contratos (nome, codigo, ativo) VALUES (?, ?, 1)", (nome, codigo))
            else:
...
def admin_editar_contrato(id):
    ...
    if request.method == 'POST':
        try:
            ...
            if 'ativo' in colunas:
                conn.execute("UPDATE contratos SET nome = ?, codigo = ?, ativo = ? WHERE id = ?", (nome, codigo, ativo, id))
            else:
                conn.execute("UPDATE contratos SET nome = ?, codigo = ? WHERE id = ?", (nome, codigo, id))
